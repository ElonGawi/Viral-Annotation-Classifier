...
class ModelReport(object):
    ...
    def show_report(self):
        ...
        print(
            f"Average time per prediction {self.avg_time_per_prediction:.5f} seconds\n"
        )
        ...
        cm_disp = ConfusionMatrixDisplay(
            ...
            display_labels=[
                AnnotationLabels.id2label[i] for i in self.cm_display_labels
            ],
        )
        cm_disp.plot(ax=cm_plot)

        ### the mem usage plot
        mem_records_df = pd.DataFrame(
            list(self.memory_records.items()), columns=["TimeDelta", "MemUsage"]
        )
        ...
        mem_records_df["MemUsageDeltaMB"] = mem_records_df["MemUsageDelta"] / (
            1024 * 1024
        )
        ...
        ### Cpu plot
        cpu_records_df = pd.DataFrame(
            list(self.cpu_usage_records.items()), columns=["TimeDelta", "CPUUsage"]
        )
...
class ModelEvaluator(object):
    def __init__(self, model, eval_dataset):
        ...
        """
        assert isinstance(
            model, ModelEvalWrapper
        ), "Model must be of a ModelEvalWrapper class"
        self.model = model
    ...
    def _profile(interval=0.01):
        ...
        def record():
            while not stop_event.is_set():
                ...
                )  # timestamp is the time delta
                memory_records[current_timestamp] = mem
                cpu_usage_records[current_timestamp] = process.cpu_percent(
                    interval=None
                )
                time.sleep(interval)
    ...
    def _predict_and_profile(self, report, profile=True, monitor_interval=0.01):
        ...
        if profile:
            stop_event, memory_records, cpu_usage_records = ModelEvaluator._profile(
                monitor_interval
            )
...
class ModelEvalWrapper(object):
    ...
    def __init__(self, model, title):
        ...
        """
        assert isinstance(
            model, ModelEvalWrapperInterface
        ), "model must implement the ModelEvalWrapperInterface Protocol"
        self.model = model
        self.title = title
